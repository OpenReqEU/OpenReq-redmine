...
classifiers = {
    ...
    "Isolation Forest": IsolationForest(max_samples=n_samples, random_state=rng)# contamination=outliers_fraction
   }
...
for j, offset in enumerate(clusters_separation):
    np.random.seed(42)
    # Data generation
    ...
    for j, (clf_name, clf) in enumerate(classifiers.items()):
        ...
        if clf_name=="Isolation Forest":
            ...
            i_outl  = scores_pred<threshold
            y_pred[i_outl] = -1
            plt.subplot(1,3,2)
            plt.plot(scores_pred)
            plt.axhline(threshold, c='r')
            plt.subplot(1,3,3)
            plt.hist(scores_pred, bins=70)
        ...
        subplot.legend(
            ...
            loc='lower right')
        subplot.set_title("%d. %s (errors: %d)" % (j + 1, clf_name, n_errors))
